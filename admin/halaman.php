    ...
    <tbody>
        ...
        if ($katakunci != '') {
            ...
            for ($x = 0; $x < count($array_katakunci); $x++) {
                $sqlcari[] = "(teacher like '%" . $array_katakunci[$x] . "%' or namayoutube like '%" . $array_katakunci[$x] . "%' or materi like '%" . $array_katakunci[$x] . "%')";
            }
            $sqltambahan    = " where " . implode(" or ", $sqlcari);
        }
        $sql1 = "select * from courses $sqltambahan order by id desc";
        $page   = isset($_GET['page']) ? (int)$_GET['page'] : 1;
        $mulai  = ($page > 1) ? ($page * $per_courses) - $per_courses : 0;
        $q1     = mysqli_query($koneksi, $sql1);
        $total  = mysqli_num_rows($q1);
        $pages  = ceil($total / $per_courses);
        $nomor  = $mulai + 1;
        $q1 = mysqli_query($koneksi, $sql1);
        $nomor = 1;
        ...
        ?>
            <tr>
                ...
                <td><?php echo $r1['teacher'] ?></td>
                <td><?php echo $r1['namayoutube'] ?></td>
